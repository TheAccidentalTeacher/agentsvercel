...
class VideoUI {
  ...
  async autoLoadTranscript(metadata) {
    ...
    try {
      ...
      const isWhisper = transcript.metadata?.source === TRANSCRIPT_SOURCE.WHISPER_AI;
      if (loadBtn) {
        if (isWhisper) {
  ...
  /**
   * Show indicator for transcript source (YouTube captions vs Whisper AI)
   */
  showTranscriptSourceIndicator(transcript) {
    ...
    const isWhisper = source === TRANSCRIPT_SOURCE.WHISPER_AI;
    ...
    if (isWhisper) {
      const cost = transcript.metadata?.costEstimate?.toFixed(4) || '0.00';
      const time = transcript.metadata?.processingTime?.toFixed(1) || '?';
      indicator.innerHTML = `
        <span class="source-badge whisper">🎤 Whisper AI</span>
        <span class="source-details">Generated in ${time}s • Est. cost: $${cost}</span>
      `;
      indicator.title = 'This video did not have YouTube captions, so we used OpenAI Whisper AI to generate a transcript';
    } else {
      indicator.innerHTML = `
    ...
    if (isWhisper) {
      ...
      sourceInfo = `
        <div class="stat-item source-stat whisper">
          ...
          <span class="stat-detail">Generated in ${time}s • $${cost}</span>
        </div>
      `;
    } else {
      sourceInfo = `
